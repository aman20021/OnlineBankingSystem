...
con = c.connect(host="localhost", user="root",
                passwd="Ta07%sur", database="bansys")
...
if  selected == "Home":
    ...
    with st.container():
        ...
        with st.container():
            ...
            contact_form = """
            <form action="https://formsubmit.co/abhishek20014@iiitd.ac.in.COM" method="POST">
...
if selected == "about-us":
    st.markdown("""
       ...
       ##### Abhishek, BTech Computer Science and Engineering, IIIT Delhi
       ##### Aman Sharma , BTech Computer Science and Engineering, IIIT Delhi
       ##### Tanay Kumar, BTech Computer Science and Engineering, IIIT Delhi    
       #### We are Online Banking Site who are working hard to bring to you the best online banking experience you can ever imagine!  
       #### :pray: :pray: :pray: **TRUST IS EARNED and with money involved it is all**:pray: :pray: :pray: 
    """, True)
...
if  selected == "Cust Login":
    ...
    if login == "Login as Customer":
        ...
        if c2.button("Submit"):
            ...
            if (bol1) and (bol2):
                ...
                #query daalna h customer ka account detail dikhane ke liye 
    ...
    if login == "Signup as Customer":
        ...
        # cacc = random.randint(25,100)
        # ccustid = random.randint(493476,993476)
        cstm = 'customer'
        cabt = accno+cid
        ...
        if c2.button("Submit"):
            ...
            #query1 = "insert into customer VALUES('{1000}' , '{cdob}' ,'{user_name}' ,'{last_name}' ,'{number}'  , '{email}' , '{cstate}','{ccity}' ,'{cpin}' ,'{cpan}' ,'{accno}')"
            #query1 = "insert into customer(cust_id,dob,first_name,last_name,phone_no,email,cust_address_state,cust_address_city,cust_address_pincode,pan_no,account_no) VALUES('{cid}', '{cdob}' ,'{user_name}' ,'{last_name}' ,'{number}'  , '{email}' , '{cstate}','{cscity}' ,'{cpin}' ,'{cpan}' ,'{accno}')"
            # "INSERT INTO  customer VALUES('{cid}' , '{cdob}' ,'{user_name}' ,'{last_name}' ,'{number}'  , '{email}' , '{cstate}','{ccity}' ,'{cpin}' ,'{cpan}' ,'{accno}')"
    ...
    if login == "Login as Employee":
        ...
        if c2.button("Submit"):
            ...
            if (bol1) and (bol2):
                ...
                #query daalna h employee ka details dikhane ke liye 
                display = "SELECT * FROM customer where email  IN ( SELECT email_id from LOGIN where username='{}')".format(userid)
                cursor.execute(display)
...
if selected == "Loan":
    ...
    if login == "Yes":
        ...
        #Loan vali query se jinke account ke pass loan h vo lao
        query="select account_no from customer"

        cursor.execute(query)
        data = cursor.fetchall()
        # list1=[]   #list containing account no
        for i in range(len(data)):
            list1.append(data[i][0])
            # list2.append(data[i][1])
        c1, c2 = st.columns([7, 1])
        if c2.button("Submit"):
            ...
            if (bol1):
                ...
                #query uss account no. se related loan ki info dikhao 
